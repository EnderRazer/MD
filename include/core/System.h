...
private:
  ...
  // Energies
  double e_pot_{}, e_kin_{}, e_int_{}, e_term_{},
      e_full_{}; // Усредненные энергии системы на 1 частицу
  double e_pot_avg_{}, e_kin_avg_{}, e_int_avg_{}, e_term_avg_{},
      e_full_avg_{}; // Усредненные энергии системы по шагам
  ...
  std::deque<double>
      temperature_window_{}; // Температура, усредненная скользящим окном

  // Давление
  double pressure_{0.0};     // Давление моментальное
  double pressure_avg_{0.0}; // Давление усредненное по шагам
  std::deque<double>
      pressure_window_{}; // Давление, усредненная скользящим окном
...
public:
  ...
  // Energies average
  inline const double ePotAvg() const {
    return e_pot_avg_ / (current_step_ + 1);
  }
  inline const double eKinAvg() const {
    return e_kin_avg_ / (current_step_ + 1);
  }
  inline const double eTermAvg() const {
    return e_term_avg_ / (current_step_ + 1);
  }
  inline const double eIntAvg() const {
    return e_int_avg_ / (current_step_ + 1);
  }
  inline const double eFullAvg() const {
    return e_full_avg_ / (current_step_ + 1);
  }

  inline const double &temperature() const { return temperature_; }
  inline const double temperatureAvg() const {
    return temperature_avg_ / (current_step_ + 1);
  }
  ...
  inline const double pressureAvg() const {
    return pressure_avg_ / (current_step_ + 1);
  }
  ...
  std::string getParticlesCoordsInfo() const {
    ...
    oss << "Coordinates:\n" << std::endl;
    for (int i = 0; i < particles_.size(); i++) {
      oss << i << ": " << particles_.coord_x_[i] << "; "
          << particles_.coord_y_[i] << "; " << particles_.coord_z_[i]
          << std::endl;
    }
  ...
  std::string getParticlesVelocityInfo() const {
    ...
    oss << "Velocity:\n" << std::endl;
    for (int i = 0; i < particles_.size(); i++) {
      oss << i << ": " << particles_.velocity_x_[i] << "; "
          << particles_.velocity_y_[i] << "; " << particles_.velocity_z_[i]
          << std::endl;
    }
  ...
  std::string getParticlesForceInfo() const {
    ...
    oss << "Force:\n" << std::endl;
    for (int i = 0; i < particles_.size(); i++) {
      oss << i << ": " << particles_.force_x_[i] << "; "
          << particles_.force_y_[i] << "; " << particles_.force_z_[i]
          << std::endl;
    }
