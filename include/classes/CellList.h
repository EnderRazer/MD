...
public:
  ...
  {
    numCells_.x() = std::max(1, static_cast<int>(std::floor(boxSize_.x() / cutoff_)));
    numCells_.y() = std::max(1, static_cast<int>(std::floor(boxSize_.y() / cutoff_)));
    numCells_.z() = std::max(1, static_cast<int>(std::floor(boxSize_.z() / cutoff_)));
  ...
  void build(const std::vector<Particle> &particles)
  {
    ...
    for (int i = 0; i < particles.size(); ++i)
    {
      ...
      {
        std::cout << "Cell index out of bounds: " << cellIdx << " For particle: " << i << " " << particles[i].coord() << std::endl;
    ...
    for (int dx = -2; dx <= 2; ++dx)
    {
      for (int dy = -2; dy <= 2; ++dy)
      {
        for (int dz = -2; dz <= 2; ++dz)
        {
          ...
          int neighborIdx = nx + ny * numCells_.x() + nz * numCells_.x() * numCells_.y();
          if (neighborIdx >= 0 && neighborIdx < totalCells_)
            neighbors.insert(neighbors.end(), cells_[neighborIdx].begin(), cells_[neighborIdx].end());
        }
      }
    }
    return neighbors;
...
private:
  ...
  {
    int x = std::min(static_cast<int>(std::floor(pos.x() / cellSize_.x())), numCells_.x() - 1);
    int y = std::min(static_cast<int>(std::floor(pos.y() / cellSize_.y())), numCells_.y() - 1);
    int z = std::min(static_cast<int>(std::floor(pos.z() / cellSize_.z())), numCells_.z() - 1);
